...
ui <- fluidPage(titlePanel("Two Proportion Resampling Test"),
                ...
                sidebarLayout(
                  sidebarPanel(
                    ...
                    checkboxInput("ShowProbability", "Show probability", FALSE),
                    conditionalPanel(condition = "input.ShowProbability == true",
                                     splitLayout(uiOutput("preprob"),
                                       textInput("cutoff", NULL),
                                       uiOutput("postprob"),
                                       textOutput("Probability"), cellWidths = c(70, 70, 30, 50)))
                  ),
...
server <- function(input, output) {
  ...
  dotplot_locs <- function(x){
    ...
    for (i in 1:length(counts)){
      ...
      if (names(counts)[i] >= input$cutoff && input$ShowProbability == TRUE){
  ...
  update_probability <- eventReactive(input$cutoff, {
  ...
  output$RandomPlot1 <- renderPlot({
    ...
    if (length(values$props) != 0){ # after reset, values$props is empty
      ...
      if (input$ShowProbability == TRUE && !is.na(as.numeric(input$cutoff))){
  ...
  output$preprob <- renderUI({
    if (input$ShowProbability){
      withMathJax('\\(P\\Big(Diff \\geq \\,\\,\\,\\)')
    }
  })
  
  output$postprob <- renderUI({
    if (input$ShowProbability){
      withMathJax('\\(\\Big)=\\)')
    }
  })
